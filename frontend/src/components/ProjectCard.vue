    ...
    <div class="p-6 space-y-4">
      ...
      <div class="space-y-2">
        <div class="flex items-start justify-between gap-2">
          <h3 class="font-black text-tactical-text-primary group-hover:text-cyber-yellow transition-colors group-hover:drop-shadow-[0_0_8px_rgba(252,238,12,0.4)] break-words" style="font-size: clamp(1rem, 1.5vw + 0.25rem, 1.25rem); line-height: 1.3;">
            {{ title }}
          </h3>
          <div class="flex-shrink-0 w-6 sm:w-8 h-1 bg-gradient-to-r from-cyber-cyan to-cyber-yellow mt-2 sm:mt-3 shadow-lg shadow-cyber-cyan/50"></div>
        </div>
      </div>

      <!-- Description -->
      <p class="text-tactical-text-secondary leading-relaxed min-h-[60px] break-words" style="font-size: clamp(0.75rem, 1vw + 0.125rem, 0.875rem);">
        {{ description }}
      </p>

      <!-- Tech Stack Pills -->
      <div v-if="techStack.length > 0" class="flex flex-wrap gap-1.5 sm:gap-2 pt-2">
        <span 
          v-for="tech in techStack" 
          :key="tech"
          class="px-2 sm:px-2.5 py-0.5 sm:py-1 font-mono font-bold bg-cyber-cyan/15 text-cyber-cyan border-2 border-cyber-cyan/40 rounded hover:bg-cyber-yellow/20 hover:text-cyber-yellow hover:border-cyber-yellow/60 transition-all shadow-lg shadow-cyber-cyan/10 hover:shadow-cyber-yellow/20 whitespace-nowrap"
          style="font-size: clamp(0.625rem, 0.9vw + 0.1rem, 0.75rem);"
        >
          {{ tech }}
        </span>
      </div>
      ...
      <div class="grid grid-cols-2 gap-3 pt-4 border-t border-tactical-border/50">
        ...
        <a 
          ...
          class="flex items-center justify-center gap-1.5 sm:gap-2 px-2 sm:px-4 py-2 sm:py-2.5 bg-cyber-cyan/15 hover:bg-cyber-yellow/20 border-2 border-cyber-cyan/60 hover:border-cyber-yellow rounded text-cyber-cyan hover:text-cyber-yellow font-mono font-black uppercase tracking-wider transition-all hover:shadow-lg hover:shadow-cyber-yellow/40 group/btn"
          style="font-size: clamp(0.625rem, 0.8vw + 0.125rem, 0.75rem);"
        >
          <ExternalLink :size="12" class="group-hover/btn:rotate-12 group-hover/btn:scale-110 transition-transform flex-shrink-0" />
        ...
        <a 
          ...
          target="_blank"
          rel="noopener noreferrer"
          class="flex items-center justify-center gap-1.5 sm:gap-2 px-2 sm:px-4 py-2 sm:py-2.5 bg-cyber-black/60 hover:bg-cyber-cyan/20 border-2 border-cyber-cyan/40 hover:border-cyber-cyan rounded text-cyber-cyan hover:text-cyber-yellow font-mono font-black uppercase tracking-wider transition-all hover:shadow-lg hover:shadow-cyber-cyan/40 group/btn"
          style="font-size: clamp(0.625rem, 0.8vw + 0.125rem, 0.75rem);"
        >
          <Github :size="12" class="group-hover/btn:rotate-12 group-hover/btn:scale-110 transition-transform flex-shrink-0" />
